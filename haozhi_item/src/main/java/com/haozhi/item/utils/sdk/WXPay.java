...
import com.haozhi.item.utils.sdk.WXPayConstants.SignType;
...
public class WXPay {
     ...
     * 作用：提现<br>
     * 场景：公共号提现
     * @param reqData 向企业支付 post的请求数据
     * @return API返回数据
     * @throws Exception
     */
    public Map<String, String> tixian(Map<String, String> reqData) throws Exception {
        return this.tixian(reqData, config.getHttpConnectTimeoutMs(), this.config.getHttpReadTimeoutMs());
    }
    ...
    public Map<String, String> tixian(Map<String, String> reqData,  int connectTimeoutMs, int readTimeoutMs) throws Exception {
        String url;
        if (this.useSandbox) {
            url = WXPayConstants.SANDBOX_TIXIAN_URL_SUFFIX;
        }
        else {
            url = WXPayConstants.TIXIAN_URL_SUFFIX;
        }
        String respXml = this.requestWithCert(url, this.fillRequestData(reqData), connectTimeoutMs, readTimeoutMs);
        return this.processResponseXml(respXml);
    }
